...
def process_image(image_in, title):
    ...
    for c in cnts:
        ...
        if w >= 30 and h >= 30 and ar >= 0.9 and ar <= 1.6:
    ...
    #grade = np.array([[ocn_quiz, quiz_title, grade]])
    # print(grade)
    #grades = np.append(grades, grade, axis=0)
    quiz_col = str(quiz_title + title)
#test if column and ocn exists, if not then add them
    if quiz_col not in grades.columns:
        grades[quiz_col] = ""
    ...
    grades.at[ocn_quiz, quiz_col] = grade
...
for pdf_file in pdf_files:
    ...
    # print(image.shape)
    image_top = image[100:280, 300:950]
    image_bottom = image[1400:1800, 300:950]

    process_image(image_top, "_top")
    process_image(image_bottom, "_bot")


grades_out = grades.sort_index()
grades_out.to_csv('grades_nom.csv')

#grades = grades[np.argsort(grades[:, 0])]
#grades = grades[np.argsort(grades[:, 1], kind='mergesort')]
print(grades_out)
